...
namespace SafeHouseAMS.WasmApp.Services
{
    ...
    internal class SurvivorCatalogueClient : GrpcClientBase, ISurvivorCatalogue
    {
        ...
        public SurvivorCatalogueClient(IMapper mapper, ILogger<SurvivorCatalogueClient> logger, GrpcChannel channel) : base(logger)
        {
            _mapper = mapper;
        ...
        public Task ApplyCommand(SurvivorCommand command, CancellationToken cancellationToken) =>
            CallHandler(async () =>
            {
                ...
                {
                    CreateSurvivor create => new CommandWrapper
                        { CreateCommand = _mapper.Map<Transport.Protos.Models.Survivors.CreateSurvivor>(create) },
        ...
        public async IAsyncEnumerable<Survivor> GetCollection(int skip, int? take, [EnumeratorCancellation] CancellationToken cancellationToken)
        {
            ...
            await CallHandler(async () =>
            {
                ...
                while (await streamingCall.ResponseStream.MoveNext())
                {
                    ...
                    result.Add(_mapper.Map<Survivor>(chunk));
                }
            });
        ...
        public Task<int> GetTotalCount() => CallHandler(async () =>
        {
